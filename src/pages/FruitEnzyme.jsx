    ...
    <Link to={path} title={isCollapsed ? label : ""}>
      <motion.div
        ...
        className={`flex items-center gap-3 px-6 py-3 my-1 transition-all duration-200 border-r-4 ${
          isActive
            ? 'bg-amber-50/80 border-amber-500 text-amber-600'
            : 'border-transparent text-gray-500 hover:bg-gray-50 hover:text-amber-600'
        } ${isCollapsed ? 'justify-center px-0' : ''}`}
      >
        <Icon size={20} strokeWidth={isActive ? 2.5 : 2} className={isActive ? "text-amber-600 flex-shrink-0" : "flex-shrink-0"} />
  ...
  return (
    <div className="flex h-screen bg-[#fcfcfb] font-sans text-gray-800 overflow-hidden">
      ...
      <aside className={`bg-white border-r border-gray-100 flex-shrink-0 hidden md:flex flex-col justify-between z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
        ...
        {/* Widget Gambar Fruit Enzyme di Bawah Sidebar */}
        {!isCollapsed && (
          <div className="px-4 pb-4">
            <div className="bg-[#fcfcfb] rounded-2xl p-2.5 border border-gray-100 overflow-hidden shadow-sm">
              <img src={fruitEnzymeImage} alt="Fruit Enzyme" className="w-full h-20 object-cover object-top rounded-xl mb-1.5" />
              <h4 className="text-[11px] font-bold text-amber-600 text-center">Fruit Enzyme Active</h4>
            </div>
          </div>
        )}
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-6 lg:px-10 z-10 flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black text-gray-900">Dashboard Fruit Enzyme</h1>
          ...
          <div className="flex items-center gap-4 lg:gap-6">
            ...
            <div className="relative">
              <select value={selectedFermenter} onChange={(e) => setSelectedFermenter(e.target.value)} className="appearance-none bg-amber-50 text-amber-600 font-bold text-xs px-4 py-2.5 pr-8 rounded-2xl border border-amber-200 outline-none cursor-pointer">
              ...
              <ChevronDown size={14} className="absolute right-3 top-3.5 text-amber-600 pointer-events-none" />
            </div>

            <button className="relative w-10 h-10 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-600 hover:text-amber-600 hover:bg-amber-50 transition-colors">
              <Bell size={18} />
              ...
              <motion.div variants={fadeInUp} className="lg:col-span-2 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <div className="relative">
                      <select value={selectedIndicator} onChange={(e) => setSelectedIndicator(e.target.value)} className="appearance-none bg-amber-50 text-amber-600 font-bold text-xs px-3.5 py-2 pr-7 rounded-xl border border-amber-200 outline-none cursor-pointer">
